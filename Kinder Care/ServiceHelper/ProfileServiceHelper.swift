...
class ProfileServiceHelper {
    ...
    class func requestFormData<T: Codable>(router: ProfileServiceManager, completion: @escaping (Result<T, CustomError>) -> ()) {
        ...
        ServiceHelper.instance.requestFormData(withData: data, forRequest: request, completion: { data, error, statusCode  in
            ...
            DispatchQueue.main.async {
                ...
                case .noContent :
                    ...
                    do {
                        if let dict = try JSONSerialization.jsonObject(with: _data, options: [.allowFragments]) as? [String : Any] {
                            ...
                            else  if let errorMessages = dict["msg"] as? String{
                                completion(.failure(.message(errorMessages)))
                            }
                            else {
                                completion(.failure(.unKnown("Please Give Valid Data")))
                ...
                case .notFound:
                    ...
                    do {
                        if let dict = try JSONSerialization.jsonObject(with: _data, options: [.allowFragments]) as? [String : Any] {
                            if let errorMessages = dict["msg"] as? String{
                                completion(.failure(.message(errorMessages)))
                            }
                            print(dict)
                    ...
                    //completion(.failure(.unKnown("Please Give Valid Data"))) is for Change password alert
    ...
    class func request<T: Codable>(router: ProfileServiceManager, completion: @escaping (Result<T, CustomError>) -> ()) {
