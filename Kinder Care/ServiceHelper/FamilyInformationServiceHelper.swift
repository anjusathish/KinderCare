//
//  FamilyInformationServiceHelper.swift
//  Kinder Care
//
...
class FamilyInformationServiceHelper {
    
    class func requestFormData<T: Codable>(router: FamilyInformationServiceManager, completion: @escaping (Result<T, CustomError>) -> ()) {
            ...
            DispatchQueue.main.async {
                ...
                case .notFound,.unAuthorized:
                    ...
                    do {
                        if let dict = try JSONSerialization.jsonObject(with: _data, options: [.allowFragments]) as? [String : Any] {
                            print(dict)
                            if let errorMessages = dict["errors"] as? NSDictionary, let key = errorMessages.allKeys.first as? String, let values = errorMessages[key] as? [String], let value = values.first {
    ...
    class func request<T: Codable>(router: FamilyInformationServiceManager, completion: @escaping (Result<T, CustomError>) -> ()) {
                ...
                case .noContent :
                    ...
                    do {
                        if let dict = try JSONSerialization.jsonObject(with: _data, options: [.allowFragments]) as? [String : Any] {
                            ...
                            else if let errorMessages = dict["pickup_person_error"] as? String {
                                    completion(.failure(.message(errorMessages)))
                                }
                            else {
                                completion(.failure(.unKnown("Please Give Valid Data")))
